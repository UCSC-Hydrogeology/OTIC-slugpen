function p51_ScreenGrab
% p51_ScreenGrab calls screencapture (embedded) to make a tiff image
%
...
%load p51_temp.mat
disp('p51_ScreenGrab: performing screengrab...')

ScreenSize      = get(0,'screensize');
ScreenSizeFull = ScreenSize;

% Full Screen Including Controls
ScreenSizeFull(4)=ScreenSizeFull(4)*0.94;

% Data Portion of Screen
ScreenSizeData = ScreenSizeFull;
ScreenSizeData(1) = ScreenSizeFull(3)/5;
ScreenSizeData(3) = ScreenSizeFull(3)-(ScreenSizeFull(3)/5);


PrintName = char(inputdlg({'Enter filename:'},'Tiff filename',[1 48],{[filename '.tiff']}));
pause(1);
% ENABLE FOR FULL SCREEN INCLUDING CONTROLS
%screencapture(0,ScreenSizeFull,PrintName)

% ENABLE FOR DATA PORTION OF THE SCREEN (NO CONTROLS)
screencapture(0,ScreenSizeData,PrintName)

% -------------------------------------------------------------------------
% Embedded Sub-Function Screencapture
% -------------------------------------------------------------------------
